...
def generate_card_caption(card: Dict, user_data: Optional[Dict] = None, count: int = 1, show_bonus: bool = False) -> str:
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        keyboard = [
            ...
            [KeyboardButton("📂 Мои карты"), KeyboardButton("👤 Мой профиль")],  # ← Добавлена кнопка
...
async def show_user_cards(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        # Находим первую карту
        card = find_card_by_id(unique_card_ids[0], data["cards"])
        if not card:
            await update.message.reply_text("Ошибка: карточка не найдена")
            return
        
        # Создаём клавиатуру с навигацией по уникальным картам
        keyboard = create_cards_keyboard(0, len(unique_card_ids))
        
        # Получаем количество дубликатов
        count = card_counts[card["id"]]
        
        # ⭐ НЕ ПОКАЗЫВАЕМ БОНУСЫ (show_bonus=False) ⭐
        ...
        logger.error(f"Ошибка при показе карт: {e}")
...
async def my_profile(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        # Процент коллекции
        collection_percent = round((unique_cards / total_available_cards * 100), 1) if total_available_cards > 0 else 0
        ...
        rarity_text = ""
        for rarity in ["T1", "T2", "T3", "T4", "T5", "T6", "T7", "T8", "UpgradeT1", "UpgradeT2"]:
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Обработчик инлайн-кнопок навигации (с учётом дубликатов)."""
    try:
        ...
        if "card_prev" in query.data or "card_next" in query.data:
            ...
            await edit_card_message(
                ...
                card,
                caption,
                create_cards_keyboard(new_index, total_cards)
            )
    except Exception as e:
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        if text == "💣 Получить карту":
            ...
            available_cards = [
                card for card in data["cards"] 
                if card["available"] and card.get("rarity") not in ["UpgradeT1", "UpgradeT2"]
            ]
...
async def craft(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        # Фильтруем только карты, которые можно крафтить (T1 или T2)
        craftable_by_rarity = {}
        for card_id, count in craftable_cards.items():
            card = find_card_by_id(card_id, data["cards"])
            if card and card.get("rarity") in ["T1", "T2"]:
        ...
        if not craftable_by_rarity:
            await update.message.reply_text(
                "❌ Для крафта подходят только карты редкости T1 или T2!"
            )
            return
...
async def process_craft(update: Update, context: ContextTypes.DEFAULT_TYPE, user_id: str, card_id: int, data: Dict, query=None) -> None:
    ...
    try:
        ...
        if source_rarity == "T1":
            target_rarity = "UpgradeT1"
        elif source_rarity == "T2":
            target_rarity = "UpgradeT2"
        else:
            if query:
                await query.edit_message_text(f"❌ Карты редкости {source_rarity} нельзя скрафтить!")
            else:
                await update.message.reply_text(f"❌ Карты редкости {source_rarity} нельзя скрафтить!")
            return
